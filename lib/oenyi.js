...
doResize = function(params, fn) {
  ...
  this._getSize(function(error, original) {
    ...
    if (params.method === 'fit') {
      imageFits = original.width === destiny.width && original.height === destiny.height;
      return resizeByFit.call(oenyi, original, destiny, imageFits, fn);
    }

    imageFits = original.width <= destiny.width && original.height <= destiny.height;
    resizeByContain.call(oenyi, original, destiny, imageFits, fn);
  });
};
...
resizeByFit = function(original, destiny, fits, fn) {
  if (fits) {
    this._calc = {resize: deepcopy(original)};
    return fn(null, this._image);
  }

  destiny = resizeUpOrDown.call(this, original, destiny);
  this._calc = {resize: deepcopy(destiny)};
  this._image.resize(destiny.width, destiny.height);
  fn(null, this._image);
};

resizeByContain = function(original, destiny, fits, fn) {
  if (fits) {
    this._calc = {resize: deepcopy(original)};
    return fn(null, this._image);
  }
